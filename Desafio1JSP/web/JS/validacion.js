function validacion()
{
    ...
    const form = document.getElementById('formulario')[0];

    const email = document.getElementById('email');
    const emailError = document.querySelector('#email + small.error');

    const dni = document.getElementById('dni');
    const dniError = document.querySelector('#dni + small.error');

    const nombre = document.getElementById('nombre');
    const nombreError = document.querySelector('#nombre + small.error');

    const apellidos = document.getElementById('apellidos');
    const apellidosError = document.querySelector('#apellidos + small.error');

    const password = document.getElementById('password');
    const passwordError = document.querySelector('#password + small.error');

    const edad = document.getElementById('edad');
    const edadError = document.querySelector('#edad + small.error');
    ...
    function errorDni() {
        if (dni.validity.valueMissing) {
            ...
            // Si el campo no contiene una dirección de correo electrónico
            // muestra el mensaje de error siguiente.
            dniError.textContent = 'El dni tiene que cumplir este patron 00000000X';
        }
    ...
    function errorPass() {
        if (password.validity.valueMissing) {
            ...
            // muestra el mensaje de error siguiente.
            passwordError.textContent = 'La contraseña debe tener al menos ' + password.minLength + ' caracteres ';
        }
